...
def update_policy(r_sa,P_ssa,V):
    ...
    for i in range(16):
        ...
        Pi_new[i, max_num] = 1
    return Pi_new
def value_iteration(Pi_0,r_sa,P_ssa,V_init):
    Pi_cur = Pi_0
    P_pi = np.zeros((16, 16))
    C_pi = np.zeros((16, 1))
    # 初始化当前值函数
    ...
    for i in range(16):
        # 计算pi(a|s)*p(s'|s,a)
        P_pi[i, :] = np.dot(np.expand_dims(Pi_cur[i, :], axis=0), P_ssa[:, i, :]).squeeze()
        # 计算pi(a|s)*r(s,a)
        C_pi[i, :] = np.dot(r_sa[i, :], Pi_cur[i, :])
    V_hat = C_pi + np.dot(P_pi, V_cur)
    delta = np.linalg.norm(V_hat - V_cur)
    # 策略改进
    Pi_new = update_policy(r_sa, P_ssa, V_hat)
    iter_num  = 1
    while delta>1e-6:
        Pi_cur = Pi_new
        V_cur = V_hat
...
if __name__ == '__main__':
    ...
    Pi_optim, iter_num = value_iteration(Pi, r_sa, P_ssa,V_init)
    print(Pi_optim, iter_num)
